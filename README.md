...
### 1. Define Linux backup account settings
...
### 2. Define MySQL backup settings
...
### 3. Define backup processing settings
...
Store the private SSH key in the repository secrets.

The public SSH key is passed to the setup script as `SSH_PUBLIC_KEY`.

### 6. Generate the MySQL backup password

Generate a password for the MySQL backup user.

Store this password in the repository secrets.

The same password is passed to the setup script as `MYSQL_PASSWORD`.
...
| -------------------- | -------: | ---------------------------------------- | ------------------------------------------------------------ |
| `BACKUP_USER`        |       No | `backup`                                 | Linux user used for SSH backup access.                       |
| `BACKUP_HOME`        |       No | `/home/$BACKUP_USER`                     | Home directory of the Linux backup user.                     |
| `BACKUP_COMMAND`     |       No | `/usr/local/sbin/github-mysql-backup.sh` | Path where the forced backup command is installed.           |
| `SSH_PUBLIC_KEY`     |      Yes | —                                        | Public SSH key allowed to execute the forced backup command. |
| `AGE_PUBLIC_KEY`     |      Yes | —                                        | Public `age` key used to encrypt backups.                    |
| `DB_NAME`            |      Yes | —                                        | MySQL database name to back up.                              |
| `MYSQL_USER`         |       No | `backup`                                 | MySQL user used for backup operations.                       |
| `MYSQL_PASSWORD`     |      Yes | —                                        | Password for the MySQL backup user.                          |
| `MYSQL_HOST_PATTERN` |       No | `localhost`                              | MySQL host pattern for the backup user.                      |
| `MYSQLDUMP_CMD`      |       No | `mysqldump`                              | Command used to create the database dump.                    |
| `MYSQL_CHARSET`      |       No | `utf8mb4`                                | MySQL client character set used during backup execution.     |
| `MYSQL_ADMIN_CMD`    |       No | `mysql`                                  | Command used to configure MySQL users and grants.            |
| `PROCESS_COMMAND`    |       No | `zstd -19`                               | Command used to process the dump stream before encryption.   |
